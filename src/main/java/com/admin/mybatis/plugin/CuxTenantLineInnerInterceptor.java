...
/**
 * <p>
 * 租户拦截器 {@link com.baomidou.mybatisplus.extension.plugins.inner.TenantLineInnerInterceptor}
 * </p>
 *
 * @author bin.xie
 * @since 2020/9/10
...
public class CuxTenantLineInnerInterceptor extends BaseMultiTableInnerInterceptor implements InnerInterceptor {

    private TenantLineHandler tenantLineHandler;

    private Set<String> msIds = null;

    public void setIgnoreTenantMsIds(List<String> ignoreTenantMsIds) {
        if (ignoreTenantMsIds != null) {
            msIds = new HashSet<>(ignoreTenantMsIds);
        }
    }
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) throws SQLException {
        ...
        if (CollUtil.isNotEmpty(msIds)) {
            Optional<String> include = msIds.stream().filter(v -> ms.getId().startsWith(v)).findFirst();
